...
class Shop {
  ...
  updateQuality() {
    ...
    for (var i = 0; i < this.items.length; i++) {
      ...
      if (this.items[i].name != 'Aged Brie' && this.items[i].name != 'Backstage passes to a TAFKAL80ETC concert' 
          && (this.items[i].quality > 0) && this.items[i].name != 'Sulfuras, Hand of Ragnaros') { 
            this.items[i].quality = this.items[i].quality - 1; // reduce quality by 1 unit
          }
      ...
      // completion of if else above and then do following below: //
    ...
    return this.items; // returns items after update method has run
  }
